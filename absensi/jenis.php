...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
		...
		echo '<h2>Daftar Jenis</h2><hr>';
      echo '<div class="row">';
		echo '<div class="col-md-7"><table class="table table-bordered">';
		echo '<tr class="info"><th>#</th><th width="200">Tahun Pelajaran</th><th>Tingkat</th><th>Jumlah Siswa</th>';
		...
		if(mysql_num_rows($sql) > 0){
			...
			while(list($tapel,$tingkat,$jumlah) = mysql_fetch_array($sql)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$tapel.'</td><td>'.$tingkat.'</td><td>'.$jumlah.'</td><td>';
