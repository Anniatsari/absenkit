	...
	*/
	...
	if(isset($_REQUEST['submit'])){
      ...
      echo '<td><a href="./cetak.php?nis='.$nis.'" target="_blank" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-print" aria-hidden="true"></span> cetak semua</a></td></tr>';
		echo '<tr><td colspan="2">Nama Siswa</td><td colspan="4">'.$idprodi.'</td></tr>';
      
		echo '<tr><td colspan="2">Absensi</td><td colspan="4">';
?>
<form class="form-inline" role="form" method="post" action="./admin.php?hlm=bayar">
  ...
  <div class="form-group">
	  ...
	  <?php
		...
		while(list($kelas,$th_pelajaran)=mysql_fetch_array($qkelas)){
			echo '<option value="'.$kelas.'">'.$kelas.' ('.$th_pelajaran.')</option>';
		}
	  ?>
	  </select>
  </div>
  ...
  <div class="form-group">
	<label class="sr-only" for="jml">Jumlah Absensi</label>
	<div class="input-group">
		<div class="input-group-addon">Ket.</div>
		<input type="text" class="form-control" id="jml" name="jml" placeholder="Jumlah Absensi">
	</div>
  </div>
  <button type="submit" class="btn btn-default" name="submit" value="bayar">Absen</button>
</form>
<?php
		echo '</td></tr>';
		echo '<tr class="info"><th width="50">#</th><th width="100">Kelas</th><th>Bulan</th><th>Tanggal Absen</th><th>Jumlah Absen</th>';
		echo '<th>&nbsp;</th>';
		echo '</tr>';
